...
class LinearSolverTestCase(unittest.TestCase, NumpyAssertions):
    ...
    def setUp(self):
        ...
        self.problinsolvers = [linear_solvers.problinsolve]  # , linear_solvers.bayescg]

        # Matrix-based linear solvers
        self.matblinsolvers = [linear_solvers.problinsolve]

        # Solution-based linear solvers
        self.solblinsolvers = [linear_solvers.bayescg]
    ...
    def test_dimension_mismatch(self):
        ...
        b = np.zeros(shape=[4])
        x0 = np.zeros(shape=[1])
        for plinsolve in [linear_solvers.problinsolve, linear_solvers.bayescg]:
            with self.subTest():
    ...
    def test_randvar_output(self):
        ...
        for plinsolve in self.problinsolvers:
            with self.subTest():
                ...
                for rv in [x, A, Ainv]:
                    self.assertIsInstance(rv, probability.RandomVariable,
    ...
    def test_posterior_distribution_parameters(self):
        ...
        # Solve linear system
        u_solver, Ahat, Ainvhat, info = linear_solvers.problinsolve(A=A, b=f, A0=Ahat0, Ainv0=Ainvhat0,
