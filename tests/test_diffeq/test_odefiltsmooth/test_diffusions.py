...
def some_meas_rv2():
    ...
    """
    some_mean = np.arange(10, 13)
    some_cov = np.arange(3, 12).reshape((3, 3)) @ np.arange(10, 19).reshape(
        (3, 3)
    ).T + np.eye(3)
    meas_rv = randvars.Normal(some_mean, some_cov)
    return meas_rv
...
class DiffusionTestInterface(abc.ABC):
    ...
    @abc.abstractmethod
    def test_estimate_locally_and_update_in_place(self, some_meas_rv1, some_meas_rv2):
        raise NotImplementedError
...
class TestConstantDiffusion(DiffusionTestInterface):
    ...
    def test_estimate_locally_and_update_in_place(self, some_meas_rv1, some_meas_rv2):
        sigma_squared = self.diffusion.estimate_locally_and_update_in_place(
            meas_rv=some_meas_rv1, meas_rv_assuming_zero_previous_cov=some_meas_rv2
        )
