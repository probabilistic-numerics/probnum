"""Test cases for `CovarianceFunction.__call__`."""
...
def fixture_call_result(
    ...
    """Result of ``CovarianceFunction.__call__`` when given ``x0`` and ``x1``."""
...
def fixture_call_result_naive(
    ...
    """Result of ``CovarianceFunction.__call__`` when applied to the entries of ``x0``
    and ``x1`` in a loop."""

    return k_call_naive(x0, x1)


def test_type(call_result: Union[np.ndarray, np.floating]):
    """Test whether the type of the output of ``CovarianceFunction.__call__`` is a NumPy
    type, i.e. an ``np.ndarray`` or a ``np.floating``."""
...
def test_shape(
    ...
    """Test whether the shape of the output of ``CovarianceFunction.__call__`` matches
    the shape of the naive reference implementation."""
...
def test_values(
    ...
    """Test whether the entries of the output of ``CovarianceFunction.__call__`` match
    the entries generated by the naive reference implementation."""
...
def test_wrong_input_dimension(
    ...
    if k.input_ndim > 0:
        ...
        with pytest.raises(ValueError):
            k(np.zeros(input_shape), None)

        with pytest.raises(ValueError):
            k(np.ones(input_shape), np.zeros(shape + k.input_shape))

        with pytest.raises(ValueError):
            k(np.ones(shape + k.input_shape), np.zeros(input_shape))
...
def test_broadcasting_error(
    ...
    with pytest.raises(ValueError):
        k(
            np.zeros(x0_shape + k.input_shape),
            np.ones(x1_shape + k.input_shape),
        )
