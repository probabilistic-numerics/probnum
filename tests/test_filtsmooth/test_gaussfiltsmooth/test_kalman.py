...
class TestKalmanDiscreteDiscrete(CarTrackingDDTestCase):
    ...
    def setUp(self):
        ...
        self.kalman = Kalman(self.dynmod, self.measmod, self.initrv)

    def test_dynamicmodel(self):
        """
        """
        self.assertEqual(self.dynmod, self.kalman.dynamicmodel)

    def test_measurementmodel(self):
        """
        """
        self.assertEqual(self.measmod, self.kalman.measurementmodel)

    def test_initialdistribution(self):
        """
        """
        self.assertEqual(self.initrv, self.kalman.initialrandomvariable)

    def test_predict(self):
        """
        """
        pred, __ = self.kalman.predict(0., self.delta_t, self.initrv)
    ...
    def test_update(self):
        ...
        upd, __, __, __ = self.kalman.update(0., self.initrv, data)
    ...
    def test_filtsmooth(self):
        ...
        """
        filtms, filtcs = gaussfilter(self.obs, self.tms, self.kalman)
        smooms, smoocs = gaussfiltsmooth(self.obs, self.tms, self.kalman)
