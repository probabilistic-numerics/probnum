...
be provided and is updated with information collected by the solvers
...
def problinsolve(
    ...
    **kwargs
    ...
    b : array_like or RandomVariable, shape=(n,) or (n, nrhs)
        ...
        posteriors over the matrices acting as priors for subsequent solves. If the
        right-hand-side is assumed to be noisy, every iteration of the solver samples a
        realization from ``b``.
    ...
    if nrhs > 1:
        # Iteratively solve for multiple right hand sides
        # (with posteriors as new priors)
        for i in range(nrhs):
            if i > 0:
...
def _check_linear_system(A, b, A0=None, Ainv0=None, x0=None):
    ...
    if Ainv0 is not None and not isinstance(Ainv0, linop_types):
        raise ValueError(
            "The inverse of A must be either an array, a linear operator or a "
            "random variable of either."
        )
...
def _init_solver(A, b, A0, Ainv0, x0, assume_A):
    ...
    if (
        ...
        warnings.warn(
            "A is assumed to be noisy, but is neither a random variable nor a linear "
            "operator. Use exact probabilistic linear solver instead."
        )
...
def _postprocess(info, A):
    ...
    if rel_cond is not None and 1 / rel_cond < machine_eps:
        msg = (
            ...
            "Results are likely inaccurate.".format(rel_cond)
        )
        warnings.warn(msg, scipy.linalg.LinAlgWarning, stacklevel=3)
