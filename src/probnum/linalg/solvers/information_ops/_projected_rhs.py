...
class ProjectedRHSInformationOp(LinearSolverInformationOp):
    ...
    Obtain information about a linear system by projecting the right hand side :math:`b=Ax` onto a given action :math:`s_i` resulting in :math:`y_i = b^\top s_i`.
    """

    def __call__(
        self, solver_state: "probnum.linalg.solvers.LinearSolverState"
    ) -> np.ndarray:
        r"""Projected right hand side :math:`b^\top s_i = (Ax)^\top s_i` of the linear system.
