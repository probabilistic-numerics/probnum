...
class ProjectedResidualInformationOp(LinearSolverInformationOp):
    ...
    Obtain information about a linear system by projecting the residual :math:`b-Ax_{i-1}`
    onto a given action :math:`s_i` resulting in :math:`s_i \mapsto s_i^\top r_{i-1}
    = s_i^\top (b - A x_{i-1}) = s_i^\top A (x - x_{i-1})`.
    """
