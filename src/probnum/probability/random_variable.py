...
class RandomVariable:
    ...
    Every probabilistic numerical method takes a random variable encoding the prior distributions as input and outputs a
    random variable whose distributions encodes the uncertainty arising from finite computation. The generic signature
    ...
    change their ``distributions`` and not necessarily all previously available methods are retained.
    ...
    def __init__(self, shape=None, dtype=None, distribution=None):
        ...
        # Set dtype (in accordance with distributions)
        self._dtype = dtype
        ...
        if distribution is not None:
            if self.dtype is None:
                ...
                # Change distributions dtype if random variable type is different
                distribution.dtype = dtype

        # Set shape (in accordance with distributions mean)
        self._shape = shape
        if distribution is not None:
            if distribution.mean is not None:
                ...
                if shape is None or shape_mean == shape:
                    ...
                    raise ValueError("Shape of distributions mean and given shape do not match.")

        # Set distributions of random variable
        if distribution is not None:
    ...
    def mean(self):
        ...
        if self._distribution is not None:
            ...
            raise NotImplementedError("No underlying distributions specified.")
    ...
    def random_state(self):
        """Random state of the random variable induced by its ``distributions``."""
    ...
    def reshape(self, shape):
        ...
        # Change distributions parameters
