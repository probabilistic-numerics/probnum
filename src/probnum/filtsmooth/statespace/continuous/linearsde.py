...
from diffeq.auxiliary.quadrature import clenshawcurtis
from diffeq.randomvariable import gaussian
from diffeq.statespace.continuousmodel import continuousmodel


class Linear(continuousmodel.ContinuousModel):
    """
...
class LTI(Linear):
    ...
    def __init__(self, driftmatrix, force, dispmatrix, diffmatrix):
        ...
        """
        Linear.__init__(self, (lambda t, *args, **kwargs: driftmatrix),
    ...
    def chapmankolmogorov(self, start, stop, step, randvar, *args, **kwargs):
        ...
        if nsteps % 2 == 0:
            nsteps = nsteps + 1
        quad = clenshawcurtis.ClenshawCurtis(nsteps, 1, np.array([[0, h]]))
        ...
        return gaussian.MultivariateGaussian(trans @ mean + force,
                                             trans @ covar @ trans.T + transdiff)
